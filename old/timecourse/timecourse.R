...
lumi.gaa <- lumi.patient[,!is.na(lumi.patient$GAA1)]
lumi.rmgaa <- removeBatchEffect(lumi.gaa, covariates = lumi.gaa$GAA1)
...
patient.out <- mb.long(lumi.rmgaa, method = "1D", type = "robust", times = 4, reps = patient.reps, rep.grp = patient.repgrp, time.grp = patient.timegrp)
...
expr.gene <- exprs(lumi.exprs.collapse["NEUROD2",])
gene.df <- data.frame(Status = lumi.exprs.collapse$Status, Expression = as.vector(expr.gene), Time.Point = lumi.exprs.collapse$Sample.Num, PIDN = lumi.exprs.collapse$PIDN)
gene.patient <- filter(gene.df, Status == "Patient")

p <- ggplot(gene.df, aes(x = Time.Point, y = Expression, group = PIDN, color = Status)) + geom_line()
p <- p + geom_smooth(mapping = aes(group = Status, method = "lm"), se = FALSE)
#p <- p + ylim(0,10)
CairoPDF("gene.plot", width = 7, height = 6)
print(p)
dev.off()
