...
#rna <- read_csv("./dan_all_rna_20150832T123409.csv")
#arrays <- read_csv("./dan_all_expression_20150821T123602.csv")
...
#dup.blocks.regex <- paste("^", dup.blocks, sep = "") %>% paste("$", sep = "") %>% paste(collapse = "|")
#dup.df <- filter(multiple.melt, grepl(dup.blocks.regex, L1)) %>% arrange(value)
#dup.PIDNs <- duplicated(dup.df$value)
#dup.PIDN.nums <- dup.df[dup.PIDNs,]$value

#No longer usable with local copy of file!
#missing.indices <- lapply(indices, function(x) {return(length(x) < 1)}) %>% reduce(c)
#bad.PIDNs <- multiple.PIDN[missing.indices] %>% reduce(rbind)
#colnames(bad.PIDNs) <- c("Good.PIDN", "Bad.PIDN")
#write.xlsx(bad.PIDNs, "bad.PIDNs.xlsx")
chop.families <- join(chop.all, families.df) %>% select(PIDN:GAA2, Site, Family)
final.block3 <- max(chop.families$Family) + 1

ucla.all <- rbind(ucla.1, ucla.2) %>% select(PIDN, sex, Status, dob, onset, gaa1, gaa2)
colnames(ucla.all) <- c("PIDN", "Sex", "Status", "DOB", "Onset", "GAA1", "GAA2")
ucla.all$Site <- "UCLA"
ucla.all$PIDN %<>% str_replace("FA_", "")
ucla.all$Family <- final.block3:(final.block3 + nrow(ucla.all) - 1)

subjects.reduce <- rbind(chop.families, ucla.all)
save(subjects.reduce, file = "subjects.reduce.rda")
#subjects.all$Status %<>% capitalize 
#save(subjects.all, file = "subjects.all.rda")

#subjects.gaa$GAA1 <- repeat1
#subjects.gaa$GAA2 <- repeat2
...
#names(targets.dates.full) %<>% str_replace_all(" ", ".")
...
#colnames(targets.dates) <- c("PIDN", "RIN", "Date.Drawn", "Date.Received", "Sample.Num")

#targets.dates %<>% arrange(PIDN)
#new.PIDN.key <- paste("^", replace.PIDNs$New, "$", sep = "")
#new.PIDN.subset <- filter(targets.dates, grepl(paste(new.PIDN.key, collapse = "|"), PIDN))

#skips <- by(new.PIDN.subset, new.PIDN.subset$PIDN, select, Sample.Num) %>% laply(Compose(as.numeric, sort, diff, find.skips))
#skips.ids <- unique(new.PIDN.subset$PIDN)[skips]
#new.PIDN.skips <- filter(new.PIDN.subset, PIDN %in% skips.ids)
#fixed.samplenums <- by(new.PIDN.skips, new.PIDN.skips$PIDN, select, Sample.Num) %>% llply(function(x){ return(1:length(x)) }) %>% reduce(c) %>% as.character 
#targets.dates[targets.dates$PIDN %in% skips.ids,]$Sample.Num <- fixed.samplenums

#Merge fixed dates
targets.extradates <- read.xlsx("../phenotypedata/missingdates-Yang.xlsx")
targets.extradates$Date.Drawn %<>% parse_date_time("mdy") %>% as.Date 
replace.conditions.first <- match(targets.extradates$Sample.Name, targets.key.all$Sample.Name)
targets.key.all[replace.conditions.first,]$Sample.Num <- targets.extradates$Sample.Num
...
#Add some missing RINS
crazy.conditions <- is.na(targets.final$RIN) & !is.na(targets.final$Date.Drawn) & grepl("NG", targets.final$Sample.Name)
targets.final[crazy.conditions,]$Sample.Num <- 1
targets.final[crazy.conditions,]$RIN <- targets.dates[match(targets.final[crazy.conditions,]$PIDN, targets.dates$PIDN),]$RIN

#Add more missing RINs
targets.extrarins <- read.xlsx("../phenotypedata/missingrins_filled.xlsx")
#missingrin.index <- tar
targets.final[is.na(targets.final$RIN),]$RIN <- targets.extrarins$RIN

#Add a few more missing draw dates
replace.conditions <- match(targets.extradates$Sample.Name, targets.final$Sample.Name)
targets.final[replace.conditions,]$Date.Drawn <- targets.extradates$Date.Drawn
#targets.final[replace.conditions,]$RIN <- targets.extradates$RIN

# targets.otherdates <- read.xlsx("../phenotypedata/missing_draw_other.xlsx")
# targets.otherdates$Date.Drawn %<>% parse_date_time("mdy") %>% as.Date 
# replace.conditions.second <- match(targets.otherdates$Sample.Name, targets.final$Sample.Name)
# targets.final[replace.conditions.second,]$Date.Drawn <- targets.otherdates$Date.Drawn

new.targets <- filter(targets.final, SCGC.Code == "2014-278")
max.date <- max(new.targets$Date.Received)
day(max.date) <- day(max.date) + 7
new.targets$Request.Date <- max.date
write.xlsx(new.targets, "2014-278.xlsx")
newer.targets <- filter(targets.final, SCGC.Code == "2015-9185")
max.date2 <- max(newer.targets$Date.Received[!is.na(newer.targets$Date.Received)])
day(max.date2) <- day(max.date2) + 7
newer.targets$Request.Date <- max.date2
write.xlsx(newer.targets, "2015-9185.xlsx")
...
#by(targets.final, targets.final$PIDN, select, Sample.Num)

#Merge phenotype data
#subjects.reduce <- select(subjects.all, FA_PatientID:dob, onset, gaa1, gaa2)
#colnames(subjects.reduce) <- c("PIDN", "Sex", "Status", "DOB", "Onset", "GAA1", "GAA2")
#subjects.reduce %<>% rbind(chop.3.reduce)
...
hell.conditions <- subjects.reduce$GAA1 < 45 & !is.na(subjects.reduce$GAA1)
...
targets.final[is.na(targets.final$Sex),]$Sex <- "UNKNOWN"
targets.final$Sex %<>% factor
...
#FA 4788 and 6707 are patients at both centers!
#TEMPORARY--- Get some phenotype data from GIFTbase
#data.temp <- read.xlsx("../phenotypedata/dan_allpatients_20151003T172042.xlsx")
#data.temp$PIDN %<>% str_replace("FA_", "")
#data.temp.reduce <- select(data.temp, PIDN, Gender, Categorized.Diagnosis)
#targets.final %<>% join(data.temp.reduce)
#hell.conditions <- is.na(targets.final$Sex) & !is.na(targets.final$Gender) & targets.final$Gender != "UNKNOWN"
#targets.final[hell.conditions,]$Sex <- targets.final[hell.conditions,]$Gender
#targets.final %<>% select(Sample.Name, PIDN, Sample.Num:GAA2)

#missingpheno.chop <- filter(targets.final, is.na(Status) & !grepl("NG", Sample.Name))
#write.xlsx(missingpheno.chop, "newpatients_chop.xlsx")
missingpheno.ucla <- filter(targets.final, is.na(Status) & grepl("NG", Sample.Name))
write.xlsx(missingpheno.ucla, "newpatients_ucla.xlsx")
targets.final[is.na(targets.final$Status),]$Status <- "Unknown"
#TEMPORARY
