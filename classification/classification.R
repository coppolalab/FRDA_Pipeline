...
lumi.import <- readRDS.gz("../baseline_lumi/save/export.lumi.rda")
...
expr.pca <- expr.collapse[,lumi.import$Status == "Patient" | lumi.import$Status == "Carrier"]
expr.pco <- expr.collapse[,lumi.import$Status == "Patient" | lumi.import$Status == "Control"]
expr.cc <- expr.collapse[,lumi.import$Status == "Control" | lumi.import$Status == "Carrier"]
...
lda.pca.cv <- crossval(lda.predfunction, t(expr.pca), lumi.pca$Status, K = 5, B = 1, negative = "Carrier")
...
lda.pco.cv <- crossval(lda.predfunction, t(expr.pco), lumi.pco$Status, K = 5, B = 1, negative = "Control")
...
lda.cc.cv <- crossval(lda.predfunction, t(expr.cc), lumi.cc$Status, K = 5, B = 1, negative = "Control")
...
dlda.pca.cv <- crossval(dlda.predfunction, t(expr.pca), lumi.pca$Status, K = 5, B = 1, negative = "Carrier")
...
dlda.pco.cv <- crossval(dlda.predfunction, t(expr.pco), lumi.pco$Status, K = 5, B = 1, negative = "Control")
...
dlda.cc.cv <- crossval(dlda.predfunction, t(expr.cc), lumi.cc$Status, K = 5, B = 1, negative = "Control")
...
rglm.pca <- randomGLM(t(expr.pca), droplevels(lumi.pca$Status), nThreads = 7)
saveRDS.gz(rglm.pca, "./save/rglm.pca.rda")
rglm.thresholds <- seq(1, 15, 1)
...
pca.rglm.df <- data.frame(Symbol = rownames(expr.pca), times.selected = as.vector(t(pca.rglm))) %>% filter(times.selected > 0) %>% arrange(desc(times.selected)) 
write.xlsx(pca.rglm.df, "pca.rglm.xlsx")

rglm.pco <- randomGLM(t(expr.pco), droplevels(lumi.pco$Status), nThreads = 7)
saveRDS.gz(rglm.pco, "./save/rglm.pco.rda")
...
pco.rglm.df <- data.frame(Symbol = rownames(expr.pco), times.selected = as.vector(t(pco.rglm))) %>% filter(times.selected > 0) %>% arrange(desc(times.selected)) 
write.xlsx(pco.rglm.df, "pco.rglm.xlsx")

rglm.cc <- randomGLM(t(expr.cc), droplevels(lumi.cc$Status), nThreads = 7)
saveRDS.gz(rglm.cc, "./save/rglm.cc.rda")
...
cc.rglm.df <- data.frame(Symbol = rownames(expr.pco), times.selected = as.vector(t(cc.rglm))) %>% filter(times.selected > 0) %>% arrange(desc(times.selected)) 
...
list.nums <- recursive.multiply(list.nums.init, nrow(expr.collapse))
list.nums <- c(nrow(expr.collapse), list.nums)
...
nfold = 5
svm.pca.input <- cbind(as.integer(droplevels(lumi.pca$Status)), t(expr.pca))
...
svm.pca.results = lapply(folds.pca.rows, svmRFE.wrap, svm.pca.input, k=5, halve.above=3)
...
svm.pco.input <- cbind(as.integer(droplevels(lumi.pco$Status)), t(expr.pco))
...
svm.pco.results = lapply(folds.pco.rows, svmRFE.wrap, svm.pco.input, k=5, halve.above=3)
...
svm.cc.input <- cbind(as.integer(droplevels(lumi.cc$Status)), t(expr.cc))
...
svm.cc.results = lapply(folds.cc.rows, svmRFE.wrap, svm.cc.input, k=5, halve.above=3)
...
pca.data <- list(x = expr.pca, y = lumi.pca$Status, geneid = rownames(expr.pca), genenames = rownames(expr.pca)) 
sc.pca <- pamr.train(pca.data)
sc.pca.cv <- pamr.cv(fit = sc.pca, data = pca.data, nfold = 5)
...
pco.data <- list(x = expr.pco, y = lumi.pco$Status, geneid = rownames(expr.pco), genenames = rownames(expr.pco)) 
sc.pco <- pamr.train(pco.data)
sc.pco.cv <- pamr.cv(fit = sc.pco, data = pco.data, nfold = 5)
...
cc.data <- list(x = expr.cc, y = lumi.cc$Status, geneid = rownames(expr.cc), genenames = rownames(expr.cc)) 
sc.cc <- pamr.train(cc.data)
sc.cc.cv <- pamr.cv(fit = sc.cc, data = cc.data, nfold = 5)
...
#Ridge Regression
ridge.pca <- cv.glmnet(x = t(expr.pca), y = droplevels(lumi.pca$Status), type.measure = "auc", nfolds = 5, family = "binomial", alpha = 0)
...
ridge.pco <- cv.glmnet(x = t(expr.pco), y = droplevels(lumi.pco$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 0)
...
ridge.cc <- cv.glmnet(x = t(expr.cc), y = droplevels(lumi.cc$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 0)
...
#Lasso Regression
lasso.pca <- cv.glmnet(x = t(expr.pca), y = droplevels(lumi.pca$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 1)
...
lasso.pco <- cv.glmnet(x = t(expr.pco), y = droplevels(lumi.pco$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 1)
...
lasso.cc <- cv.glmnet(x = t(expr.cc), y = droplevels(lumi.cc$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 1)
...
elasticnet.pca <- cv.glmnet(x = t(expr.pca), y = droplevels(lumi.pca$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 0.5)
...
elasticnet.pco <- cv.glmnet(x = t(expr.pco), y = droplevels(lumi.pco$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 0.5)
...
elasticnet.cc <- cv.glmnet(x = t(expr.cc), y = droplevels(lumi.cc$Status), family = "binomial", type.measure = "auc", nfolds = 5, alpha = 0.5)
...
#Random KNN
rknn.pca <- rknnBeg(t(expr.pca), droplevels(lumi.pca$Status), pk = 0.8, r = 1000 )
...
rknn.pco <- rknnBeg(t(expr.pco), droplevels(lumi.pco$Status), pk = 0.8, r = 1000 )
...
rknn.cc <- rknnBeg(t(expr.cc), droplevels(lumi.cc$Status), pk = 0.8, r = 1000 )
...
accuracies.table <- rbind(rf.accuracy, rglm.accuracies, svm.accuracies / 100, sc.accuracies, ridge.accuracies, lasso.accuracies, elasticnet.accuracies, rknn.accuracies, lda.accuracies, dlda.accuracies) %>% data.frame 
...
CairoPDF("pca.accuracies", height = 5, width = 7)
print(pca.plot)
dev.off()
